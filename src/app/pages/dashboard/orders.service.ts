 ...
 */
function sort(orders: any, column: string, direction: string): Order[] {
    if (direction === '') {
        return orders;
    } else {
...
function matches(orders: Order, term: string, pipe: PipeTransform) {
    return orders.SNo.toLowerCase().includes(term)
        || orders.createdDate.toLowerCase().includes(term)
        || orders.lastExecutedDate.toLowerCase().includes(term)
        || orders.modelName.toLowerCase().includes(term)
        || orders.healthstatus.toLowerCase().includes(term);
}
